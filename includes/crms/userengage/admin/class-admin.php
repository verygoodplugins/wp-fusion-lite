...
class WPF_UserEngage_Admin {
	...
	public function validate_import_trigger( $input, $setting ) {
		...
		$prev_value = wp_fusion()->settings->get('userengage_add_tag');
		...
		$rule_id = wp_fusion()->settings->get('userengage_add_tag_rule_id');

		if( ! empty( $rule_id ) ) {
			wp_fusion()->crm->destroy_webhook( $rule_id );
			add_filter( 'validate_field_userengage_add_tag_rule_id', function() { return false; } );
		}
		...
		add_filter( 'validate_field_userengage_add_tag_rule_id', function() use (&$rule_id) { return $rule_id; } );
	...
	public function validate_update_trigger( $input, $setting ) {
		...
		$prev_value = wp_fusion()->settings->get('userengage_update_trigger');
		...
		// See if we need to destroy an existing webhook before creating a new one
		$rule_id = wp_fusion()->settings->get('userengage_update_trigger_rule_id');

		if( ! empty( $rule_id ) ) {
			wp_fusion()->crm->destroy_webhook($rule_id);
			add_filter( 'validate_field_userengage_update_trigger_rule_id', function() { return false; } );
		}
		...
		add_filter( 'validate_field_userengage_update_trigger_rule_id', function() use (&$rule_id) { return $rule_id; } );
